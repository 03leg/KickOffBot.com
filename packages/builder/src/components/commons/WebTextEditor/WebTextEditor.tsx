            ...
            <Box sx={{ height: '100%', border: `1px solid ${Colors.BORDER}`, padding: ({ spacing }) => (spacing(1)), display: 'flex', flexDirection: 'column', position: 'relative' }}>
                ...
                <Box sx={{ display: 'flex', marginTop: ({ spacing }) => (spacing(2)), justifyContent: 'flex-end' }}>
                    ...
                    <VariableSelectorDialog
                        newVariableTemplate={{ type: VariableType.STRING, value: 'Hello world!' }}
                        onInsertVariable={handleInsertVariable} supportPathForObject={true} availableVariableTypes={[VariableType.STRING, VariableType.DATE_TIME, VariableType.NUMBER, VariableType.BOOLEAN, VariableType.OBJECT]} />
