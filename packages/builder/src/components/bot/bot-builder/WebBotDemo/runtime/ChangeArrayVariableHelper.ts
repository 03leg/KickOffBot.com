...
export class ChangeArrayVariableHelper {
  public static getArrayValue(workflow: ChangeArrayVariableWorkflow, variable: BotVariable, userContext: WebUserContext, utils: WebBotManagerUtils) {
    const sourceVariableValue = userContext.getVariableValueByName(variable.name) as unknown[];

    // console.log(sourceVariableValue);

    if (!Array.isArray(sourceVariableValue)) {
      throw new Error("InvalidOperationError: sourceVariableValue is not array");
    }
    ...
    switch (workflow.operation) {
      case ChangeArrayOperation.Add: {
        ...
        let values = this.getArrayValueByPath(workflow.addDescription.variableSourceDescription.path, userContext, utils);

        values = this.applyFilter(userContext, utils, values, workflow.addDescription.variableSourceDescription.extraFilter);
      ...
      case ChangeArrayOperation.Set: {
        ...
        let values = this.getArrayValueByPath(workflow.setDescription.variableSourceDescription.path, userContext, utils);

        values = this.applyFilter(userContext, utils, values, workflow.setDescription.variableSourceDescription.extraFilter);
      ...
      case ChangeArrayOperation.Remove: {
        ...
        for (const value of sourceVariableValue) {
          ...
          if (workflow.removeDescription.logicalOperator === LogicalOperator.AND && checks.every((p) => p)) {
            removedItems.push(value);
          } else if (workflow.removeDescription.logicalOperator === LogicalOperator.OR && checks.some((p) => p)) {
            removedItems.push(value);
          }
        }
  ...
  private static applyFilter(
    ...
    const result = this.checkConditions(userContext, utils, values, filter.conditions, filter.logicalOperator);
  ...
  private static checkValue(userContext: WebUserContext, utils: WebBotManagerUtils, condition: PropertyConditionItem, value: unknown): boolean {
    throwIfNil(condition.operator);

    const conditionValue = this.getConditionValue(userContext, utils, condition);
  ...
  private static getConditionValue(userContext: WebUserContext, utils: WebBotManagerUtils, condition: PropertyConditionItem): unknown {
    ...
    if (!isNil(condition.variableIdValue)) {
      ...
      if (!isNil(condition.pathVariableIdValue) && typeof variableValue === "object" && !Array.isArray(variableValue)) {
  ...
  public static getArrayValueByPath(source: ValuePathDescription, userContext: WebUserContext, utils: WebBotManagerUtils): unknown[] {
  ...
  public static getVariableValue(source: ValuePathDescription, userContext: WebUserContext, utils: WebBotManagerUtils) {
    ...
    const variableValue = userContext.getVariableValueByName(sourceVariable.name);

    let value: unknown;

    if (!isNil(path) && typeof variableValue === "object" && !Array.isArray(variableValue)) {
