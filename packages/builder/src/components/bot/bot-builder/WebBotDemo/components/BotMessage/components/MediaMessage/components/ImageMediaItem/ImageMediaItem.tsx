...
export const ImageMediaItem = ({ image, isLast, direction, wrapped, count }: Props) => {
    ...
    return (
        <Box className={classes.imageContainer} onMouseEnter={() => setShowPhotoInfo(true)}
            ...
            {image.isLink ? <a href={image.imageLink} target='_blank'> <img className={classes.img} src={imageUrl} /></a> : <img className={classes.img} src={imageUrl} />}
        </Box>
    )
}
